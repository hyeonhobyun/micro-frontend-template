...
import {
  ...
  spriteRootDirectory,
  svgRootDirectory,
} from 'libs/constants/src/directory';
import { join } from 'path';

if (!existsSync(spriteRootDirectory)) {
  mkdirSync(spriteRootDirectory, { recursive: true });
}

if (!existsSync(svgRootDirectory)) {
  mkdirSync(svgRootDirectory, { recursive: true });
}

// if (!existsSync(assetTypeHelperPath)) {
//   writeFileSync(assetTypeHelperPath, '', 'utf8');
// }
...
const getSVGs = readdirSync(svgRootDirectory, {
  withFileTypes: true,
})
